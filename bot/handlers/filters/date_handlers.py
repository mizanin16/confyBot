...
async def process_calendar_interaction(callback: CallbackQuery, callback_data: SimpleCalendarCallback, state: FSMContext):
    ...
    if selected:
        ...
        if date < today:
            await callback.answer("🚫 Нельзя выбрать прошедшую дату! Выберите дату начиная с сегодняшнего дня.", show_alert=True)
        ...
        if await state.get_state() == FilterStates.date_custom.state:
            ...
            if start_date > date:
                await callback.answer("🚫 Конечная дата не может быть раньше начальной! Выберите другую дату.", show_alert=True)
                # ❗ Снова показываем календарь
